...
type : all , users , group , channel

all untuk semua, users untuk user, group untuk group, channel untuk channel (admin saja)
...
<blockquote><b>{ktrn} ᴋᴇᴛ: sᴜᴅᴀʜ ᴀᴅᴀ ᴅᴀʟᴀᴍ ʟɪsᴛ ʙʟᴀᴄᴋʟɪsᴛ</blockquote></b>
"""
        else:
            await add_to_vars(client.me.id, "BL_ID", chat_id)
            txt = f"""
<blockquote><b>{grp} ɢʀᴏᴜᴘ: {message.chat.title}</blockquote></b>\n<blockquote><b>{ktrn} ᴋᴇᴛ: sᴜᴄᴄᴇs ᴀᴅᴅ ғᴏʀ ʙʟᴀᴄᴋʟɪsᴛ</blockquote></b>
"""
...
async def _(client, message):
    ...
    try:
        ...
        if chat_id not in blacklist:
            response = f"""
<blockquote><b>{grp} ɢʀᴏᴜᴘ: {message.chat.title}</blockquote></b>
<blockquote><b>{ktrn} ᴋᴇᴛ: ᴛɪᴅᴀᴋ ᴀᴅᴀ ᴅᴀʟᴀᴍ ʟɪsᴛ ᴛᴇᴍᴘᴇᴋ</b></blockquote>
"""
        else:
            await remove_from_vars(client.me.id, "BL_ID", chat_id)
            response = f"""
<blockquote><b>{grp} ɢʀᴏᴜᴘ: {message.chat.title}</blockquote ></b>
<blockquote><b>{ktrn} ᴋᴇᴛ: sᴜᴄᴄᴇssғᴜʟʟʏ ʀᴇᴍᴏᴠᴇᴅ ғʀᴏᴍ ᴛʜᴇ ʙʟᴀᴄᴋʟɪsᴛ</blockquote></b>
"""
...
async def _(client, message):
    ...
    ggl = await EMO.GAGAL(client)   
    msg = await message.reply(f"{prs}proses bang...")
